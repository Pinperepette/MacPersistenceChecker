...
final class RiskScorer {
    ...
    private enum Weight {
        ...
        // Launch Frequency Anomaly risks
        static let microRestart: Int = 20           // StartInterval < 60 sec
        static let aggressiveWatchdog: Int = 25     // KeepAlive + StartInterval < 30 sec
        static let noThrottleWithKeepAlive: Int = 10 // KeepAlive without ThrottleInterval
    ...
    private func assessBehaviorRisks(_ item: PersistenceItem) -> [RiskDetail] {
        ...
        // MARK: Launch Frequency Anomaly Detection

        // Micro-restart: StartInterval < 60 seconds is suspicious
        if let interval = item.startInterval, interval < 60 {
            risks.append(RiskDetail(
                factor: "Micro-Restart",
                points: Weight.microRestart,
                description: "StartInterval=\(interval)s - executes every <60s (possible watchdog)"
            ))
        }

        // Aggressive watchdog: KeepAlive + very low StartInterval
        if item.keepAlive == true, let interval = item.startInterval, interval < 30 {
            risks.append(RiskDetail(
                factor: "Aggressive Watchdog",
                points: Weight.aggressiveWatchdog,
                description: "KeepAlive + StartInterval=\(interval)s - aggressive persistence pattern"
            ))
        }

        // KeepAlive without ThrottleInterval (can cause rapid restart loops on crash)
        if item.keepAlive == true && item.throttleInterval == nil {
            // Only flag if not Apple-signed (legitimate apps often do this)
            if item.signatureInfo?.isAppleSigned != true {
                risks.append(RiskDetail(
                    factor: "No Throttle Limit",
                    points: Weight.noThrottleWithKeepAlive,
                    description: "KeepAlive without ThrottleInterval - can cause crash-restart loops"
                ))
            }
        }
